...
def handle_data_editor(df, sheet, key_prefix, df_session_key):
    ...
    edited_df = st.data_editor(
        df_display, key=f"{key_prefix}_editor", use_container_width=True, hide_index=True,
        column_config={
            ...
            if st.button("💾 確認修改", key=f"{key_prefix}_save"):
                try:
                    ...
                    if not deleted_rows.empty:
                        for idx in deleted_rows.index: rows_to_delete.append(idx + 2)
                        for row_num in sorted(rows_to_delete, reverse=True): sheet.delete_rows(row_num)
                    ...
                    for idx, row in edited_df.iterrows():
                        ...
                        for col in cols:
                            ...
                            if isinstance(new_val, (datetime, pd.Timestamp)): new_val = new_val.strftime("%Y-%m-%d")
                            if isinstance(old_val, (datetime, pd.Timestamp)): old_val = old_val.strftime("%Y-%m-%d")
                            if str(new_val) != str(old_val): changes_found = True
...
def show_income_tab(sheet_income, df_income, income_types):
    ...
    with st.expander("➕ 新增收入", expanded=False):
        ...
        # 動態分類
        ADD_NEW = "➕ 新增類別..."
        type_opts = income_types + [ADD_NEW]
        
        sel_type = st.selectbox("類別", type_opts)
        new_type = None
        if sel_type == ADD_NEW:
            new_type = st.text_input("輸入新收入類別")
        ...
        if st.button("📥 存入"):
            if sheet_income:
                ...
                # 更新 Setting
                if sel_type == ADD_NEW and new_type and new_type not in income_types:
                    update_setting_value("Income_Types", ",".join(income_types + [new_type]))
                    st.toast(f"已新增收入分類：{new_type}")

                st.success("已存入！")
                load_all_finance_data.clear() # 清除快取以便下次 Setting 更新
                if "fin_data_loaded" in st.session_state: del st.session_state["fin_data_loaded"]
                st.rerun()
